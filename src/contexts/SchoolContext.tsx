...
export interface Membership { school_id: string; role: Role }
...
export function SchoolProvider({ children }: { children: ReactNode }) {
  ...
  const loadMemberships = useCallback(async (uid: string) => {
    const { data } = await supabase.from("memberships").select("school_id,role").eq("user_id", uid).eq("status", "active");
