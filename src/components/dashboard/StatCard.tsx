...
export function StatCard({ label, value, trend, sub, icon: Icon, tone = "info" }: Props) {
  const t = TONE[tone];
  return (
    <div className="group rounded-xl bg-card border border-border p-4 sm:p-5 shadow-card hover:shadow-soft transition-all">
      <div className="flex items-start justify-between">
        <div className="min-w-0">
          <div className="text-xs font-medium text-muted-foreground">{label}</div>
          <div className="mt-1.5 sm:mt-2 text-xl sm:text-[28px] font-display font-bold tracking-tight">{value}</div>
        </div>
        <div className={cn("size-9 sm:size-11 grid place-items-center rounded-lg shrink-0", t.bg)}>
          <Icon className={cn("size-4 sm:size-5", t.fg)} />
        </div>
      </div>
      {trend && (
