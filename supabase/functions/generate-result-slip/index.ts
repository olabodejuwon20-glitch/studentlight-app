  ...
  try {
    ...
    if (qrBytes) {
      try {
        ...
        page.drawText("Scan to verify authenticity", { x: 32, y: qy + qSize - 6, size: 9, font: bold, color: ink });
        page.drawText("This QR confirms the result was issued by", { x: 32, y: qy + qSize - 22, size: 8, font, color: muted });
        page.drawText(`${school?.name ?? "the school"} and has not been altered.`, { x: 32, y: qy + qSize - 34, size: 8, font, color: muted });
